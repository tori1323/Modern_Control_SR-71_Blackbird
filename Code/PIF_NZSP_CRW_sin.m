function [gains,table,sys] = PIF_NZSP_CRW_sin(sys_temp,pacing,table,t_final,ym,dis)
...
% Begin Code::
    ...
    % Preliminaries
    [sys_old,sys,gains] = Build_Discrete_System_PIF_CRW(sys_temp.A,sys_temp.B,sys_temp.C,...
    ...
    % Initial Variables
    table.u(:,1) = (pi22*gains.K(:,N+1) + gains.K(:,(1:N))*pi12)*ym(1) - gains.K * table.x(:,1); % Update control values
    ...
    for i = 1:(length(table.t)-1)
        ...
        table.x(:,i + 1) = A * table.x(:,i) + B * table.u(:,i) - ...
            (Y*ym(i)) + dis * h; % Update state values
        
        if sum(ismember(table.t(1,i+1),frames)) > 0
            table.u(:,i+1) = (pi22*gains.K(:,N+1) + gains.K(:,(1:N))*pi12)*ym(i) - gains.K * table.x(:,i+1); % Update control values
        else
    ...
    table.u(:,length(table.t)) = (pi22*gains.K(:,N+1) + gains.K(:,(1:N))*pi12)*ym(end) - gains.K * table.x(:,end); % Update control values
