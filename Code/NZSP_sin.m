function [gains,table] = NZSP_sin(sys_temp,pacing,table,t_final,ym,H)
...
% Begin Code::
    ...
    [pi12,pi22] = QPMCALC(A-eye(size(A)),B(:,2),H,0);
    table.pi12 = pi12;
    table.pi22 = pi22;
    
    % Initial Variables
    table.u(:,1) = (pi22 + gains.K*pi12)*ym(1) - gains.K * table.x(:,1); % Update control values
    ...
    for i = 1:(length(table.t)-1)
        ...
        if sum(ismember(table.t(1,i+1),frames)) > 0
            table.u(:,i+1) = (pi22 + gains.K*pi12)*ym(i+1) - gains.K * table.x(:,i+1); % Update control values
        else
    ...
    table.u(:,length(table.t)) = (pi22 + gains.K*pi12)*ym(end) - gains.K * table.x(:,end); % Update control values
